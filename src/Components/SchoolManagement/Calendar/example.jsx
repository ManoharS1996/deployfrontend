...
const CustomToolbar = ({ label, onNavigate, onView, view, date }) => {
  ...
  const handleTodayClick = () => {
    ...
    onNavigate(today); 
    onView("month"); // Ensure the view is set to "month" after navigating to today
  };
...
const Example = () => {
  ...
  const [currentDate, setCurrentDate] = useState(new Date(today.getFullYear(), today.getMonth(),today.getDay())); // Track the currently selected date
  ...
  useEffect(() => {
    ...
    // Re-filter whenever allevents changes
    setFilteredEvents(allevents => {
      if (currentView === "agenda") {
        return filterEventsForAgenda(currentDate);
      } else {
        return allevents;
      }
    });
  ...
  const handleViewChange = (view) => {
    if (view === "agenda") {
      // When switching to "agenda" view, 
      // ensure the calendar re-renders with the filtered events
      filterEventsForAgenda(currentDate); 
    }
    setCurrentView(view);
  };
  ...
  const handleNavigate = (newDate) => {
    ...
    // Filter events only when necessary (e.g., for "agenda" view or month change)
    if (currentView === "agenda" || 
        newDate === "PREV" || 
        newDate === "NEXT") { 
      filterEventsForAgenda(currentDate); 
